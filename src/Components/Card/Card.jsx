...
const Card = ({ cardDatapromise }) => {
	...
	return (
		...
		<div>
			...
			<div className='flex flex-col md:flex-row max-w-[1200px] mx-auto justify-between mt-8 gap-8 px-4'>
					...
					<h1 className='text-2xl'>Task Status</h1>
					{
						selectedTask.length === 0 ? (
							...
							selectedTask.map((task) => (
								<div key={task.id} className="card w-80 md:w-90 bg-base-100 card-xs shadow-sm">
...

						{
							resolve.length === 0 ?
								...
								resolve.map((task) => (
									...
									<div className="card-body">
										<h2 className='font-semibold text-2xl text-left' key={task.id}>{task.title}</h2>
									</div>
								))
						}
					</div>
